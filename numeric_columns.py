...
# ────────────────────────────────────────────────────────────────────
#  Contextual Column Boosting (Reclassification Only)
# ────────────────────────────────────────────────────────────────────
COLUMN_BOOST_THRESHOLD = 0.90  # 90% of column tokens must be trusted


@dataclass
class ColumnBoostResult:
    """Result of column-level trust boosting."""
    column_id: int
    total_tokens: int
    trusted_tokens: int
    trust_ratio: float
    boosted_tokens: List[Token] = field(default_factory=list)
    eligible: bool = False


def compute_column_boost(
    columns: List[NumericColumn],
    token_trust_map: dict,
    threshold: float = COLUMN_BOOST_THRESHOLD,
) -> List[ColumnBoostResult]:
    """Identify columns eligible for contextual boosting.

    If >= 90% of tokens in a vertical column are TRUSTED
    (LOCKED, SURYA_VALID, or CNN_CONFIRMED), the remaining
    tokens are reclassified as NUMERIC_CANDIDATE for re-validation.

    FORBIDDEN:
      - Copying digits from neighbors
      - Forcing numeric values
      - Context-only digit creation

    Args:
        columns: Detected numeric columns.
        token_trust_map: Dict mapping token key (bbox tuple) to trust
                         status string. Keys are (x0, y0, x1, y1).
        threshold: Minimum fraction of trusted tokens (default 0.90).

    Returns:
        List of ColumnBoostResult with tokens eligible for re-validation.
    """
    results = []
    ...
    for col in columns:
        total = col.count
        if total < 2:
            results.append(ColumnBoostResult(
                column_id=col.column_id,
                total_tokens=total,
                trusted_tokens=0,
                trust_ratio=0.0,
        ...
        for tok in col.tokens:
            ...
            trust = token_trust_map.get(key, "UNTRUSTED")
            if trust in ("LOCKED", "SURYA_VALID", "CNN_CONFIRMED"):
                trusted_count += 1
            else:
                untrusted_tokens.append(tok)

        ratio = trusted_count / total if total > 0 else 0.0
        eligible = ratio >= threshold and len(untrusted_tokens) > 0

        results.append(ColumnBoostResult(
            column_id=col.column_id,
            total_tokens=total,
            trusted_tokens=trusted_count,
            trust_ratio=round(ratio, 3),
            boosted_tokens=untrusted_tokens if eligible else [],
            eligible=eligible,
        ))
